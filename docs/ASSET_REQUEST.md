...
- **アイコン 78/87 受領・組み込み済み**
- **背景 8/8 受領・組み込み済み**
- **アプリアイコン / OGP 受領・組み込み済み**（favicon・PWAマニフェスト設定済み）
- キャラクター：先輩・高尾 受領済み／八ヶ岳・箱根 **未着**

### まだ届いていないアイコン 9種（現在は暫定SVGで表示中）

`trash` `scissors` `grass` `footsteps` `tools` `sprout` `log` — 山整備画面の作業7種
`map` `water` — ショップの地図・水分

### 差し替え推奨（意図と絵柄がずれているもの）

| ファイル名 | 現状 | 本来の想定 |
|---|---|---|
| `socks.webp` | 丸めたタオル/寝袋に見える | 登山用ソックス |
| `breath.webp` | 赤ちゃんが息を吐く絵 | 首ストレッチ（大人が首を回す） |

※ 上記も同名で置き直せば自動で差し替わります。
